...
**Host:** `hlh-docker` LXC 109 `192.168.1.9/24` on `prox01` (`hlh-docker/deploy-hlh-docker.sh:44`)
...
**Status:** Stage 0 plan + Stage 1 build complete. `--plan` validates on laptop, `--apply` runs on prox01 via `pct exec 109`.

## 1. Host Inventory (verified)

* `hlh-docker` pure-bash, no Terraform/Ansible `hlh-docker/README.md:2`. Provisions unprivileged LXC 109 `hlh-docker/deploy-hlh-docker.sh:317` with `nesting=1,keyctl=1`, `4c/4096MB/32GB` on `RaidZ1-6TB` `hlh-docker/deploy-hlh-docker.sh:322`.
...
```
 ...
 Engine LXC 113 (vLLM       192.168.1.13:8000 /metrics)
                         │
                         ▼
          hlh-docker LXC 109 (192.168.1.9) ── macvlan ──► 192.168.1.14
                            /srv/data (mp0)
...
* **Storage:** No new ZFS dataset. Reuse `hlh-docker-data` at `/srv/data/grafana-prometheus`. Two bind volumes `docker-compose.yml:48` via `driver_opts: type none, device: /srv/data/...` — survives `pct destroy 109` per `ADR-001.md:36`.
...
* `deploy-hlh-grafana-prometheus.sh` — pure-bash `pct exec 109`, `--plan` (laptop-friendly) / `--apply` / `--nuke` / `--help`, colour helpers like `hlh-docker/deploy-hlh-docker.sh:69`, `pct push` with base64 fallback, macvlan create guard, `chown 472:472`, health checks

## 5. Execution Flow

1. **Pre-flight on prox01:** `pct status 109` running, `docker --version`, `docker network inspect macvlan` or create `docker network create -d macvlan --subnet 192.168.1.0/24 --gateway 192.168.1.1 -o parent=eth0 macvlan`, `docker compose config` validate.
2. **Plan (laptop or prox01):** `./deploy-hlh-grafana-prometheus.sh --plan` — validates compose locally, prints plan, no `pct` required.
3. **Apply (on prox01):** `mkdir -p /srv/data/grafana-prometheus/{prometheus_data,grafana_data}` via `pct exec 109`, `chown 472:472`, `pct push` compose+configs, `docker compose pull && down --remove-orphans && up -d`.
...
6. **Persistence gate:** `pct stop/start 109` → volumes retained on `RaidZ1-6TB/hlh-docker-data`.
...
* No changes to `hlh-docker` LXC definition — service deploys into running `109` (preserves `hlh-docker/README.md:18` boundary).
...
pct exec 109 -- docker exec prometheus wget -qO- http://localhost:9090/api/v1/targets | head -c 2000
...
* `bash -n deploy-hlh-grafana-prometheus.sh` OK, `docker compose config` OK, `--plan` passes on laptop (pct not required), `--apply` gates on pct/has `109` checks `deploy-hlh-grafana-prometheus.sh:116`.
